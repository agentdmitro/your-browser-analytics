...
## Architecture
- src/background/index.js: MV3 service worker. Fetches history, computes analytics,
  caches results, handles runtime messages.
- src/background/categorization.js: category rules + categorize helpers.
- src/popup/index.html + src/popup/index.js + src/popup/styles.css: quick stats UI.
- src/dashboard/index.html + src/dashboard/index.js + src/dashboard/styles.css: full analytics UI.
...
- src/background/index.js runs fetchHistoryData() which:
...
## Categorization
- CATEGORY_RULES in src/background/categorization.js define regex rules per category.
- categorize(domain, url) also uses NEWS_PATH_PATTERNS for path-based news.
- If you add/remove categories, also update:
  - getCategoryColor/getCategoryIcon in src/dashboard/index.js
